...
add_action('admin_notices', function () {
    ...
    if (!$screen || $screen->id !== 'settings_page_cinatra') {
        return;
    }
...
function cinatra_validate_instance_url(string $raw): string {
    ...
    if ($scheme === 'https') {
        // ok
    } elseif ($scheme === 'http' && $is_loopback) {
 ...
 */
add_action('admin_post_cinatra_connect_start', function () {
    if (!current_user_can('manage_options')) {
        wp_die(esc_html__('You do not have permission to do this.', 'cinatra'), '', ['response' => 403]);
    }
    check_admin_referer('cinatra_connect_start');

    $raw  = isset($_POST['cinatra_connect_url']) ? wp_unslash($_POST['cinatra_connect_url']) : '';
 ...
 */
add_action('admin_post_cinatra_connect_callback', function () {
    if (!current_user_can('manage_options')) {
        wp_die(esc_html__('You do not have permission to do this.', 'cinatra'), '', ['response' => 403]);
    }
    ...
    $result = cinatra_connect_exchange((string) $stored['instance_url'], [
 ...
 */
add_action('admin_post_cinatra_connect_install_code', function () {
    if (!current_user_can('manage_options')) {
        wp_die(esc_html__('You do not have permission to do this.', 'cinatra'), '', ['response' => 403]);
    }
    check_admin_referer('cinatra_connect_install_code');

    $raw = isset($_POST['cinatra_connection_string']) ? wp_unslash($_POST['cinatra_connection_string']) : '';
    ...
    $result = cinatra_connect_exchange($parsed['instance_url'], [
...
function cinatra_connect_exchange(string $instance_url, array $body): array {
    ...
    $response = wp_safe_remote_post($base . '/api/connect/token', [
        'timeout' => 15,
        'headers' => [
    ...
    if (is_wp_error($response)) {
        error_log('[cinatra] connect token exchange transport error: ' . $response->get_error_message());
        return ['ok' => false, 'response' => null];
    }
    $status = (int) wp_remote_retrieve_response_code($response);
    $json   = json_decode((string) wp_remote_retrieve_body($response), true);
    if ($status < 200 || $status >= 300 || !is_array($json) || empty($json['credential'])) {
        error_log('[cinatra] connect token exchange failed (HTTP ' . $status . ')');
        return ['ok' => false, 'response' => null];
    }
...
function cinatra_enqueue_widget(): void {
    ...
    wp_add_inline_style('cinatra-fallback', sprintf(
    ...
    wp_localize_script('cinatra-fallback', 'CinatraFallback', [
        ...
        [
            ...
            'args'                => [
                ...
                'target_url' => [
                    ...
                    'validate_callback' => function ($value) {
                        $clean = esc_url_raw((string) $value, ['http', 'https']);
                        return is_string($clean) && $clean !== '';
                    },
                ],
                'post_types' => [
...
function cinatra_rest_mint_token(WP_REST_Request $request): WP_REST_Response {
    ...
    if (empty($nonce) || !wp_verify_nonce($nonce, 'wp_rest')) {
        return new WP_REST_Response(['error' => __('Invalid or missing nonce.', 'cinatra')], 403);
    }
    ...
    if (empty($url) || empty($api_key)) {
        return new WP_REST_Response(
            ['error' => __('Cinatra URL or API key is not configured.', 'cinatra')],
            500
        );
    }
    ...
    if (empty($origin)) {
        return new WP_REST_Response(
            ['error' => __('Could not derive this site origin.', 'cinatra')],
            500
        );
    }
    ...
    $response = wp_safe_remote_post($token_endpoint, [
        'timeout' => 10,
        'headers' => [
    ...
    if (is_wp_error($response)) {
        ...
        return new WP_REST_Response(
            ['error' => __('Could not reach the Cinatra instance. Check the connector URL, or contact your administrator.', 'cinatra')],
            502
        );
    }
    ...
    if ($status < 200 || $status >= 300 || !is_array($json) || empty($json['token'])) {
        ...
        return new WP_REST_Response(
            ['error' => __('Cinatra could not issue a session token. Check the connector settings, or contact your administrator.', 'cinatra')],
            502
        );
    }
...
function cinatra_rest_create_webhook(WP_REST_Request $request): WP_REST_Response {
    ...
    $post_types = array_values((array) ($request->get_param('post_types') ?? []));

    if ($event_type === '' || $target_url === '') {
        return new WP_REST_Response(
            ['error' => __('event_type and target_url are required.', 'cinatra')],
            400
        );
    }
    ...
    if (count($existing) >= CINATRA_MAX_WEBHOOK_SUBSCRIPTIONS) {
        return new WP_REST_Response(
            ['error' => __('Maximum number of webhook subscriptions reached.', 'cinatra')],
            409
        );
    }
...
function cinatra_rest_delete_webhook(WP_REST_Request $request): WP_REST_Response {
    ...
    if (count($updated) === count($current)) {
        return new WP_REST_Response(['error' => __('Subscription not found.', 'cinatra')], 404);
    }
