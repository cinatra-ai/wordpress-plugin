...
const MUTATIONS = [
  ...
  // 7g — the storage channel (cinatra#2683). The ban became a channel, so each
  // wall of that channel gets its own negative control: a second storage touch,
  // a write that skips the shape bound, a write that skips the credential guard,
  // a read that returns whatever was stored, a key that stops naming the user,
  // and the correlationId being persisted instead of minted per document.
  ["7g  touch localStorage a second time, outside the one accessor",
    (s) => s.replace("  function readStoredThreadId(key) {\n    var store = threadStore();",
                     "  function readStoredThreadId(key) {\n    var store = window.localStorage;")],
  ["7g  persist a value without bounding its SHAPE",
    (s) => s.replace("    if (typeof id !== 'string' || !ID_PATTERN.test(id)) return;\n", "")],
  ["7g  persist without the credential guard (storage is where a credential would REST)",
    (s) => s.replace("    if (containsCredentialShapedValue(id)) return;\n", "")],
  ["7g  return whatever was stored, unbounded, into the frame's strict schema",
    (s) => s.replace("    if (typeof entry.id !== 'string' || !ID_PATTERN.test(entry.id)) return null;",
                     "    if (typeof entry.id !== 'string') return null;")],
  ["7g  key the remembered thread by the site alone (one browser profile, two people)",
    (s) => s.replace("    var user = cmsUserKey();\n    if (!user) return null;\n", "    var user = 'any';\n")],
  ["7g  persist the bridge correlationId itself (a nonce that outlives its document)",
    (s) => s.replace("    correlationId = mintCorrelationId();", "    correlationId = resolveThreadId();")],
  ["7g  reach for sessionStorage",
    (s) => s.replace("      var s = window.localStorage;", "      var s = window.sessionStorage;")],
  ["7g  drop the INSTANCE ORIGIN from the key (two deployments share one bucket)",
    (s) => s.replace("    return THREAD_STORE_PREFIX + '|' + cinatraOrigin + '|' + instanceId +",
                     "    return THREAD_STORE_PREFIX + '|' + instanceId +")],
  ["7g  drop the ASSISTANT from the key (the two CMS personas share one bucket)",
    (s) => s.replace("      '|' + EMBED_ASSISTANT + '|' + user;", "      '|' + user;")],
  ["7g  treat anonymous (uid 0) as a person",
    (s) => s.replace("    if (!key || key === '0') return '';", "    if (!key) return '';")],
  ["7g  add a SECOND storage write outside the guarded one",
    (s) => s.replace("  function resolveThreadId() {",
                     "  function rememberSomethingElse(k, v) { var st = threadStore(); if (st) { try { st.setItem(k, v); } catch (_) {} } }\n  function resolveThreadId() {")],
  ["7g  stop bounding the entry's age (an unusable thread never ages out)",
    (s) => s.replace("    if (age < 0 || age > THREAD_STORE_MAX_AGE_MS) return null;\n", "")],
  ["7g  refresh the entry on every USE (the expiry resets and never arrives)",
    (s) => s.replace("    if (stored) return stored.id;",
                     "    if (stored) { writeStoredThreadId(key, stored.id); return stored.id; }")],
];
